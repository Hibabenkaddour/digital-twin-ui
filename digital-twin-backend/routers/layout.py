...
async def layout_from_prompt(
    ...
    try:
        ...
        # Persist new state scoped to this user
        crud.save_layout(db, result.newState, user_id=user_id)
        return result
...
def get_layout_state(
    ...
    """Get saved layout state from DB — scoped to the authenticated user."""
    db_layout = crud.get_layout(db, layout_id, user_id=user_id)
    if not db_layout:
...
def get_default_layout(
    ...
    """Get default layout state — scoped to the authenticated user."""
    db_layout = crud.get_layout(db, "default", user_id=user_id)
    if not db_layout:
...
def save_layout_state(
    ...
    """Save/update layout state — scoped to the authenticated user."""
