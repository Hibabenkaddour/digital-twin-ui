...
import { checkBackendHealth, layoutPrompt } from '../services/api';
...
export default function GridStep() {
  const {
    selectedDomain, components, connections, gridCols, gridRows,
  ...
  const handleAiPrompt = async () => {
    ...
    const currentState = {
      id: 'default', name: twinName || 'Digital Twin',
      domain: selectedDomain || 'factory',
      gridCols: gridCols || 10, gridRows: gridRows || 8,
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '7px 14px', borderBottom: '1px solid var(--border)', background: 'var(--bg-1)', display: 'flex', alignItems: 'center', gap: '10px', flexShrink: 0, flexWrap: 'wrap' }}>
          ...
          {blueprints.map(bp => (
            <button key={bp.type} onClick={() => handleAdd(bp.type)} title={`Add ${bp.name}`}
              style={{
                ...
                borderRadius: '7px', cursor: 'pointer', fontSize: '11px', fontWeight: 500, transition: 'all 0.18s',
        ...
        <div style={{ display: 'flex', gap: '2px', background: 'var(--bg-0)', borderRadius: '7px', padding: '2px', flexShrink: 0 }}>
          {VIEWS.map(v => (
            <button key={v} onClick={() => setView(v)}
              style={{ padding: '3px 9px', borderRadius: '5px', border: 'none', cursor: 'pointer', fontSize: '11px', fontWeight: 500, transition: 'all 0.15s', background: view === v ? 'var(--accent)' : 'transparent', color: view === v ? '#fff' : 'var(--text-2)' }}>
              {v}
            </button>
          ))}
        </div>
        ...
        {/* AI Toggle */}
        <button onClick={() => setShowAiBar(p => !p)}
          style={{ padding: '3px 11px', borderRadius: '7px', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: '5px', fontSize: '11px', fontWeight: 600, cursor: 'pointer', flexShrink: 0, background: showAiBar ? 'rgba(72,101,242,0.15)' : 'transparent', color: showAiBar ? 'var(--accent)' : 'var(--text-2)' }}>
          🦙 AI Layout
      ...
      {showAiBar && (
        <div style={{ padding: '8px 14px', borderBottom: '1px solid var(--border)', background: 'rgba(72,101,242,0.03)', flexShrink: 0 }}>
          <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
            ...
            <div style={{ flex: 1 }}>
              <textarea value={aiPrompt} onChange={e => setAiPrompt(e.target.value)}
                ...
                placeholder='Describe layout changes, e.g. "Add 3 conveyor belts next to the shipping dock"'
                rows={2}
              ...
              <div style={{ display: 'flex', gap: '5px', marginTop: '5px', flexWrap: 'wrap' }}>
                {['Add a fuel tank 3x2', 'Add a radar dome', 'Add a control tower 2x3', 'Connect all components in sequence'].map(ex => (
          ...
          {aiResult && (
            <div style={{ marginTop: '6px', padding: '7px 12px', borderRadius: '7px', fontSize: '11px', background: aiResult.error ? 'rgba(245,158,11,0.08)' : 'rgba(16,217,141,0.08)', border: `1px solid ${aiResult.error ? 'rgba(245,158,11,0.25)' : 'rgba(16,217,141,0.25)'}`, color: aiResult.error ? '#f59e0b' : '#10d98d' }}>
