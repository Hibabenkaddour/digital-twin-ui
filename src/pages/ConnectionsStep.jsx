...
import { ChevronRight, ArrowLeft, Link2, Trash2 } from 'lucide-react';
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '320px 1fr', overflow: 'hidden' }}>
        ...
        <div style={{ borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'rgba(255,255,255,0.85)' }}>
          <div style={{ padding: '20px', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
            ...
            <div style={{ padding: '12px', borderRadius: '8px', background: 'rgba(72,101,242,0.06)', border: '1px solid rgba(72,101,242,0.15)' }}>
          ...
          <div style={{ flex: 1, overflow: 'auto', padding: '16px' }}>
            {connections.length === 0 ? (
              <div style={{ textAlign: 'center', color: 'var(--text-2)', fontSize: '13px', padding: '40px 20px' }}>
                No connections yet.<br />Drag from one component to another to link them.
              </div>
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                    ...
                    <button
                      ...
                      style={{ background: 'transparent', border: 'none', color: 'var(--text-2)', cursor: 'pointer', padding: '4px', display: 'flex', alignItems: 'center', borderRadius: '4px' }}
                  ...
                  <div style={{ marginTop: '8px', height: '3px', borderRadius: '2px', background: 'var(--bg-4)', overflow: 'hidden' }}>
                    <div style={{ height: '100%', width: conn.flowStatus === 'green' ? '80%' : conn.flowStatus === 'orange' ? '50%' : '20%', background: `linear-gradient(90deg, ${color}, transparent)`, transition: 'width 0.5s' }} />
                  </div>
                </div>
              );
            })}
          ...
          <div style={{ padding: '16px', borderTop: '1px solid var(--border)', flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', gap: '8px', flexDirection: 'column' }}>
              {[{status:'green',label:'Fluid Flow'},{status:'orange',label:'Congested'},{status:'red',label:'Bottleneck'}].map(l => (
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative' }}>
          <div style={{ position: 'absolute', top: 12, right: 12, zIndex: 10, display: 'flex', gap: '2px', background: 'rgba(255,255,255,0.7)', backdropFilter: 'blur(10px)', border: '1px solid var(--border)', borderRadius: '7px', padding: '2px' }}>
            {VIEWS.map(v => (
              <button key={v} onClick={() => setView(v)}
                style={{ padding: '4px 10px', borderRadius: '5px', border: 'none', cursor: 'pointer', fontSize: '11px', fontWeight: 600, transition: 'all 0.15s', background: view === v ? 'var(--accent)' : 'transparent', color: view === v ? '#fff' : 'var(--text-2)' }}>
                {v}
              </button>
            ))}
          </div>
          ...
          <div style={{ position: 'absolute', bottom: '16px', left: '50%', transform: 'translateX(-50%)', padding: '8px 16px', borderRadius: '100px', background: 'rgba(255,255,255,0.8)', border: '1px solid var(--border)', backdropFilter: 'blur(10px)', fontSize: '12px', color: 'var(--text-1)', zIndex: 10, pointerEvents: 'none' }}>
            {view === '2D Grid' ? '🖱️ Drag from one component to another to link them' : '🔗 Connections rendered as animated tubes'}
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 20px', borderTop: '1px solid var(--border)', background: 'rgba(255,255,255,0.8)', backdropFilter: 'blur(10px)', flexShrink: 0 }}>
        ...
        <button className="btn btn-primary" onClick={() => setStep(4)}>Next: Configure KPIs <ChevronRight size={16} /></button>
      </div>
    </div>
  );
}
